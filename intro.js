...
async function execute(interaction) {
  ...
  // 명령어를 실행한 채널에 완료 메세지 (본인에게만 보임)
  await interaction.reply({ content: '✅ 자기소개가 완료되었습니다!', ephemeral: true });
  ...
  try {
    ...
    if (isImage) {
      ...
      logEmbed.addFields({ name: '첨부파일', value: `[${attachment.name}](${attachment.url})` });
    }
